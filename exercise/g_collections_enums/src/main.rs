...
impl Shot {
    ...
    fn points(self) -> i32 {
        ...
        match self {
            Shot::Bullseye => 5,
            Shot::Hit(x) if x < 3.0 => 2,   // Using a guard is cleaner than using the if..else
            Shot::Hit(x) => 1, // Using a guard is cleaner than using the if..else
            _ => 0,   // In all other cases
        }
    }
}
